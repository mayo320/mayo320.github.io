...
function scoreUnitText(unit, text) {
    ...
    const debuff_sub_criterias = [
        new SC(/(immune|IMM)/gi, (a, b) => a * 1.45, 'defense-immune'),
        new SC(/resist/gi, (a, b) => a * 1, 'defense-resist', [
...
function calculateUnitScores(index, print=false) {
    ...
    const calcAct = (action, multiplier = 1) => {
        ...
        base_scores[action] = base_scores[action].length === 0 ? [['', 0]] : base_scores[action];
